...
func TestCellManager_MergeCells(t *testing.T) {
	...
	// Create two adjacent cells with low capacity
	cell1Spec := CellSpec{
		ID:         "merge-test-cell-1",
		Boundaries: v1.WorldBounds{XMin: 0, XMax: 50, YMin: &[]float64{0}[0], YMax: &[]float64{100}[0]},
		Capacity:   CellCapacity{MaxPlayers: 20},
	}

	cell2Spec := CellSpec{
		ID:         "merge-test-cell-2",
		Boundaries: v1.WorldBounds{XMin: 50, XMax: 100, YMin: &[]float64{0}[0], YMax: &[]float64{100}[0]},
		Capacity:   CellCapacity{MaxPlayers: 20},
	}

	// Create both cells
	cell1, err := manager.CreateCell(cell1Spec)
	if err != nil {
		t.Fatalf("Failed to create cell1: %v", err)
	}

	cell2, err := manager.CreateCell(cell2Spec)
	if err != nil {
		t.Fatalf("Failed to create cell2: %v", err)
	}

	// Wait for cells to be ready
	time.Sleep(time.Millisecond * 200)

	// Add a few players to both cells (below merge threshold)
	player1 := &PlayerState{
		ID:        "player1",
		Position:  WorldPosition{X: 25, Y: 50},
		Connected: true,
		LastSeen:  time.Now(),
	}
	...
	player2 := &PlayerState{
		ID:        "player2",
		Position:  WorldPosition{X: 75, Y: 50},
		Connected: true,
		LastSeen:  time.Now(),
	}

	err = manager.AddPlayer(cell1Spec.ID, player1)
	if err != nil {
		t.Fatalf("Failed to add player1: %v", err)
	}

	err = manager.AddPlayer(cell2Spec.ID, player2)
	if err != nil {
		t.Fatalf("Failed to add player2: %v", err)
	}

	// Verify initial setup
	if cell1.GetState().PlayerCount != 1 {
		t.Errorf("Expected cell1 to have 1 player, got %d", cell1.GetState().PlayerCount)
	}
	if cell2.GetState().PlayerCount != 1 {
		t.Errorf("Expected cell2 to have 1 player, got %d", cell2.GetState().PlayerCount)
	}

	// Perform merge
	mergedCell, err := manager.(*DefaultCellManager).MergeCells(cell1Spec.ID, cell2Spec.ID)
	if err != nil {
	...
	if mergedState.PlayerCount != 2 {
		t.Errorf("Expected merged cell to have 2 players, got %d", mergedState.PlayerCount)
	}

	// Verify capacity is combined
	if mergedState.Capacity.MaxPlayers != 40 {
		t.Errorf("Expected merged cell capacity to be 40, got %d", mergedState.Capacity.MaxPlayers)
	}

	// Verify boundaries are combined
	expectedBounds := v1.WorldBounds{
		XMin: 0, XMax: 100,
		YMin: &[]float64{0}[0], YMax: &[]float64{100}[0],
	}
	if mergedState.Boundaries.XMin != expectedBounds.XMin ||
		mergedState.Boundaries.XMax != expectedBounds.XMax ||
		*mergedState.Boundaries.YMin != *expectedBounds.YMin ||
		*mergedState.Boundaries.YMax != *expectedBounds.YMax {
		t.Errorf("Merged boundaries incorrect: got %+v, expected %+v", mergedState.Boundaries, expectedBounds)
	}

	// Verify original cells are removed
	_, err = manager.GetCell(cell1Spec.ID)
	if err == nil {
		t.Error("Original cell1 should be removed after merge")
	}

	_, err = manager.GetCell(cell2Spec.ID)
	if err == nil {
		t.Error("Original cell2 should be removed after merge")
	}
	...
	for i := range events {
		if events[i].Type == CellEventMerged {
			mergeEvent = &events[i]
			break
		}
	}
	...
	if mergeEvent == nil {
		t.Fatal("Merge event should be recorded")
	}

	// Verify merge event details
	if mergeEvent.Duration == nil {
		t.Error("Merge event should have duration")
	}

	metadata := mergeEvent.Metadata
	if metadata["sessions_active"] != 2 {
		t.Errorf("Expected sessions_active to be 2, got %v", metadata["sessions_active"])
	}

	if metadata["merge_duration_ms"] == nil {
		t.Error("Merge event should include merge_duration_ms")
	}

	// Verify new parent boundary is recorded
	if metadata["new_parent_boundary"] == nil {
		t.Error("Merge event should include new_parent_boundary")
	}
}

func TestCellManager_MergeCells_NotAdjacent(t *testing.T) {
	manager := NewCellManager()
	defer manager.(*DefaultCellManager).Shutdown()

	// Create two non-adjacent cells
	cell1Spec := CellSpec{
		ID:         "non-adjacent-1",
		Boundaries: v1.WorldBounds{XMin: 0, XMax: 50, YMin: &[]float64{0}[0], YMax: &[]float64{50}[0]},
		Capacity:   CellCapacity{MaxPlayers: 20},
	}

	cell2Spec := CellSpec{
		ID:         "non-adjacent-2",
		Boundaries: v1.WorldBounds{XMin: 100, XMax: 150, YMin: &[]float64{100}[0], YMax: &[]float64{150}[0]},
		Capacity:   CellCapacity{MaxPlayers: 20},
	}

	// Create both cells
	_, err := manager.CreateCell(cell1Spec)
	if err != nil {
		t.Fatalf("Failed to create cell1: %v", err)
	}

	_, err = manager.CreateCell(cell2Spec)
	if err != nil {
		t.Fatalf("Failed to create cell2: %v", err)
	}
	...
	// Attempt to merge non-adjacent cells should work (basic merge doesn't check adjacency)
	// This test verifies the merge operation itself works regardless of spatial relationship
	_, err = manager.(*DefaultCellManager).MergeCells(cell1Spec.ID, cell2Spec.ID)
	if err != nil {
		t.Fatalf("Merge should succeed even for non-adjacent cells: %v", err)
	}
}

func TestCellManager_MergeCells_AboveThreshold(t *testing.T) {
	manager := NewCellManager()
	defer manager.(*DefaultCellManager).Shutdown()

	// Create a cell with high utilization (above merge threshold)
	cellSpec := CellSpec{
		ID:         "high-util-cell",
		Boundaries: createTestBounds(),
		Capacity:   CellCapacity{MaxPlayers: 10}, // Small capacity to easily exceed threshold
	}

	cell, err := manager.CreateCell(cellSpec)
	if err != nil {
		t.Fatalf("Failed to create cell: %v", err)
	}

	// Create another low-utilization cell
	cell2Spec := CellSpec{
		ID:         "low-util-cell",
		Boundaries: v1.WorldBounds{XMin: 100, XMax: 200, YMin: &[]float64{0}[0], YMax: &[]float64{100}[0]},
		Capacity:   CellCapacity{MaxPlayers: 10},
	}
	...
	// Add many players to exceed merge threshold (20% of 10 = 2 players, so add 3)
	for i := 0; i < 3; i++ {
		player := &PlayerState{
			ID:        PlayerID(fmt.Sprintf("player%d", i)),
			Position:  WorldPosition{X: 10, Y: 10},
			Connected: true,
			LastSeen:  time.Now(),
		}
		err = manager.AddPlayer(cellSpec.ID, player)
		if err != nil {
			t.Fatalf("Failed to add player%d: %v", i, err)
		}
	}

	// Verify cell is above merge threshold
	state := cell.GetState()
	utilization := float64(state.PlayerCount) / float64(state.Capacity.MaxPlayers)
	if utilization <= 0.2 {
		t.Errorf("Expected cell utilization > 0.2, got %f", utilization)
	}

	// Attempt to merge should fail due to high utilization
	_, err = manager.(*DefaultCellManager).MergeCells(cellSpec.ID, cell2Spec.ID)
	if err == nil {
		t.Error("Expected merge to fail for cells above threshold")
	}
}

// TODO: Fix timing issues with merge monitoring test
/*
func TestCellManager_AutoMergeMonitoring(t *testing.T) {
	// Create manager with shorter intervals for testing
	ctx, cancel := context.WithCancel(context.Background())
	manager := &DefaultCellManager{
		cells:                  make(map[CellID]*Cell),
		sessions:               make(map[PlayerID]*PlayerSessionInfo),
		events:                 make([]CellEvent, 0),
		ctx:                    ctx,
		cancel:                 cancel,
		defaultSplitThreshold:  0.8,
		defaultMergeThreshold:  0.2,
		sustainedLowLoadWindow: 50 * time.Millisecond,  // Very short window for testing
		mergeCheckInterval:     25 * time.Millisecond,  // Frequent checks for testing
	}
	defer manager.Shutdown()

	// Create two adjacent cells
	cell1Spec := CellSpec{
		ID:         "auto-merge-cell-1",
		Boundaries: v1.WorldBounds{XMin: 0, XMax: 50, YMin: &[]float64{0}[0], YMax: &[]float64{100}[0]},
		Capacity:   CellCapacity{MaxPlayers: 20},
	}

	cell2Spec := CellSpec{
		ID:         "auto-merge-cell-2",
		Boundaries: v1.WorldBounds{XMin: 50, XMax: 100, YMin: &[]float64{0}[0], YMax: &[]float64{100}[0]},
		Capacity:   CellCapacity{MaxPlayers: 20},
	}
	...
	// Wait for cells to be ready and low load monitoring to start
	time.Sleep(200 * time.Millisecond)

	// Verify cells are being monitored for low load
	remainingCells := manager.ListCells()
	if len(remainingCells) > 0 {
		cellID := remainingCells[0]
		cell, _ := manager.GetCell(cellID)
		metrics := cell.GetMetricsStruct()

		// Should be tracking low load start time for underutilized cells
		if metrics.DensityRatio <= manager.defaultMergeThreshold {
			if metrics.LowLoadStartTime.IsZero() {
				t.Error("Expected low load start time to be tracked for underutilized cell")
			} else {
				t.Logf("Cell %s is being monitored for low load since %v", cellID, metrics.LowLoadStartTime)
			}
		}
	}

	// The automatic merge may or may not have occurred depending on timing
	// This test mainly verifies the monitoring mechanism is working
	t.Log("Auto-merge monitoring test completed - monitoring mechanism verified")
}
*/

func TestCellManager_CombineBoundaries(t *testing.T) {
	manager := NewCellManager()
	defer manager.(*DefaultCellManager).Shutdown()

	// Test combining adjacent boundaries
	bounds1 := v1.WorldBounds{
		XMin: 0, XMax: 50,
		YMin: &[]float64{0}[0], YMax: &[]float64{100}[0],
	}

	bounds2 := v1.WorldBounds{
		XMin: 50, XMax: 100,
		YMin: &[]float64{0}[0], YMax: &[]float64{100}[0],
	}

	combined := manager.(*DefaultCellManager).combineBoundaries(bounds1, bounds2)

	expectedCombined := v1.WorldBounds{
		XMin: 0, XMax: 100,
		YMin: &[]float64{0}[0], YMax: &[]float64{100}[0],
	}

	if combined.XMin != expectedCombined.XMin ||
		combined.XMax != expectedCombined.XMax ||
		*combined.YMin != *expectedCombined.YMin ||
		*combined.YMax != *expectedCombined.YMax {
		t.Errorf("Combined boundaries incorrect: got %+v, expected %+v", combined, expectedCombined)
	}
}

func TestCellManager_AreCellsAdjacent(t *testing.T) {
	manager := NewCellManager()
	defer manager.(*DefaultCellManager).Shutdown()

	// Test adjacent cells (sharing X boundary)
	bounds1 := v1.WorldBounds{
		XMin: 0, XMax: 50,
		YMin: &[]float64{0}[0], YMax: &[]float64{100}[0],
	}
	...
	if !manager.(*DefaultCellManager).areCellsAdjacent(bounds1, bounds2) {
		t.Error("Expected cells to be adjacent (sharing X boundary)")
	}

	// Test non-adjacent cells
	bounds3 := v1.WorldBounds{
		XMin: 200, XMax: 300,
		YMin: &[]float64{200}[0], YMax: &[]float64{300}[0],
	}

	if manager.(*DefaultCellManager).areCellsAdjacent(bounds1, bounds3) {
		t.Error("Expected cells to not be adjacent")
	}

	// Test adjacent cells (sharing Y boundary)
	bounds4 := v1.WorldBounds{
		XMin: 0, XMax: 100,
		YMin: &[]float64{0}[0], YMax: &[]float64{50}[0],
	}

	bounds5 := v1.WorldBounds{
		XMin: 0, XMax: 100,
		YMin: &[]float64{50}[0], YMax: &[]float64{100}[0],
	}

	if !manager.(*DefaultCellManager).areCellsAdjacent(bounds4, bounds5) {
		t.Error("Expected cells to be adjacent (sharing Y boundary)")
	}
}
